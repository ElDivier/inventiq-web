...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
          ...
          <div>
            ...
            <nav className="space-y-2">
              {menu.map(item => {
                ...
                return (
                  <button key={item.label} onClick={() => setActive(item.label)} className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left transition ${isActive ? 'bg-emerald-500/80 shadow-lg' : 'text-emerald-50 hover:bg-white/10'}`}>
      ...
      <MobileBottomNav menu={menu} active={active} setActive={setActive} />
...
function MobileBottomNav({ menu, active, setActive }) {
  const mobileMenu = menu.filter(item => ['Inicio', 'Ventas', 'Productos', 'Inventario', 'Reportes'].includes(item.label));
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 px-2 py-2 shadow-[0_-8px_25px_rgba(15,23,42,0.08)] backdrop-blur lg:hidden">
      <div className="grid grid-cols-5 gap-1">
        {mobileMenu.map(item => {
          ...
          const isActive = active === item.label;
          return (
            <button key={item.label} onClick={() => setActive(item.label)} className={`flex flex-col items-center justify-center rounded-2xl px-2 py-2 text-[11px] font-semibold transition ${isActive ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500 hover:bg-slate-50'}`}>
